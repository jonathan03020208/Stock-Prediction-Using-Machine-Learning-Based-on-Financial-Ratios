### Before running, plz setup the feild [GRU] in config.ini
### python train_GRU.py 2019-3
...
#device
if config['GRU']['device'].lower() == 'cpu':
    device = torch.device("cpu")
elif config['GRU']['device'].lower() == 'gpu':
...
#ifRelative
if config['GRU']['relative'].lower() == 'true':
    ifRelative = True
elif config['GRU']['relative'].lower() == 'false':
    ifRelative = False
else:
    raise RuntimeError('relative must be true or false')

#dir root
root = config['GRU']['model_path_root'] + '/' + sys.argv[1] + '/'
...
if ifRelative:
    val_dataset = dataset('GRU', ratio_scope, stock_relative_return, val_index)
    train_dataset = dataset('GRU', ratio_scope, stock_relative_return, train_index)
else:
    val_dataset = dataset('GRU', ratio_scope, stock_return, val_index)
    train_dataset = dataset('GRU', ratio_scope, stock_return, train_index)
...
if ifDummy:
    model = GRU(NumberOfFeatures + 8, 36, 1, dropout=0.2)
else:
...
model_path_root = config['GRU']['model_path_root']
