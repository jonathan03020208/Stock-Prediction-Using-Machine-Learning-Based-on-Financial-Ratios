### Before running, plz setup variable:config[FNN][feature_selection_index] and config[FNN][dummy]
### generate company_buy from 2019-3~2022-1 
### python generate_company_buy_FNN.py path=?
...
for simulation_num in range(27, 38):
    ...
    if ifDummy:
        model = FNN(NumberOfFeatures + len(sector_dict) - 1, 30, 15)
    else:
        model = FNN(NumberOfFeatures, 30, 15)
...
with open("company_buy_index_2.pickle", "wb") as fp:
    pickle.dump(company_buy_index, fp)
